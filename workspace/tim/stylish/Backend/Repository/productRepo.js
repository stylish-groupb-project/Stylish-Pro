...
module.exports = {
  ...
  insertNewProduct: async (res, productDataObj, mainImageUrl, connection) => {
    try {
      const {
        category,
        title,
      ...
      const insertProductQuery =
        "INSERT INTO product(category,title,description,price,texture, wash, place, note, story ,main_image) VALUES(?,?,?,?,?,?,?,?,?,?)";
  ...
  getSecKill: async (res) => {
    const connection = await connectionPromise;
    try {
      const selectQuery = "SELECT * FROM secKill";
      const [result] = await connection.execute(selectQuery);
      return result;
    } catch (error) {
      console.error(error);
