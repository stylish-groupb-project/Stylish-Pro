...
module.exports = {
  getProductDetail: async (res, sql_condition_obj, productRedisKey) => {
    const result = await productRepo.getProductByCondition(
      res,
      sql_condition_obj,
      productRedisKey
    );
    if (result.length == 0)
      return res.status(403).json("product doesn't exist");
    return result;
  },
  ...
  insertNewProduct: async (
    ...
    try {
      ...
      const result = await productRepo.insertNewProduct(
        res,
        productDataObj,
        mainImageUrl,
        connection
      ...
      const insertConcurrency = [
        colorRepo.insertColors(
          res,
          productDataObj.colors,
          newProductId,
          connection
        ),
        sizeRepo.insertSizes(
          res,
          productDataObj.sizes,
          newProductId,
          connection
        ),
        variantRepo.insertVariants(
          res,
  ...
  getSecKill: async (res) => {
    const result = await productRepo.getSecKill(res);
    return result;
  },
};
